...
define("DB_NAME", "form"); // set database name
...
function page_protect() {
    ...
    if (!isset($_SESSION['user_id'])) {
        if (isset($_COOKIE['user_id']) && isset($_COOKIE['user_key'])) {
            ...
            if (!empty($ckey) && is_numeric($_COOKIE['user_id']) && $_COOKIE['user_key'] == sha1($ckey)) {
            ...
            header("Location: ../account/submitUpdate.php");
            exit();
        }
    }
}
...
function logout() {
    ...
    if (isset($sess_user_id) || isset($cook_user_id)) {
        mysql_query("UPDATE 'users' SET 'ckey'= '', 'ctime'= '' WHERE 'id'='$sess_user_id' OR  'id' = '$cook_user_id'") or die(mysql_error());
    }
